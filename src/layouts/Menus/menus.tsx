...
const Index = () => {
    ...
    const menusClick = (e: any) => {
        const reEach = (arr: any) => {
            arr.forEach((element: any) => {
                if( element.id === e.key ){
                    const codeStr = element.code.substring(2, element.code.length);
    ...
    const eachMenusActive = (data: any) => {
        ...
        const reFunc = (arr: any) => {
            arr.forEach((v: any, k:number)=>{
                if( setSelectKey.defaultOpenSelectKey === undefined || setSelectKey.defaultSelectedKeys === undefined ){
                    // 浏览器历史记录URL访问时，初始化菜单选中状态
                    const codePath = v.code.replace(/:/g, "/").replace('m/', "/");
                    const name1 = codePath?.split('/').filter((res:any)=>res!=='')[0];
                    const name2 = history.location.pathname.split('/').filter(res=>res!=='')[0];
                    if(name1 === name2){
                        if( !v?.parent_id ){ setSelectKey.defaultOpenSelectKey = v.id }
                        if(codePath === history.location.pathname){
                            setSelectKey = { defaultOpenSelectKey: v.parent_id, defaultSelectedKeys: v.id }
                        }else {
                            if( v?.children?.length > 0 ){
                                const name3 = history.location.pathname.split('/').filter(res=>res!=='')[1];
                                if( v.children.filter((res: any)=>res.path === name3).length <= 0 ){ setSelectKey.defaultSelectedKeys = v.id }
                            }
                        }
                    }
                    if(codePath !== history.location.pathname && v?.children?.length > 0) { 
    ...
    return <div className={styles.leftMenusDiv} style={{width: commonState.collapsed ? 50 : 250}}>       
        <div className={styles.menusDiv}>
            <Spin spinning={state.menusLoads} indicator={<LoadingOutlined />}>
                <div className={styles.menusUser}>
                    <p>
                        <Iconfont type='icon-user-avatar' style={{fontSize: 16,marginRight: 5}} /> 
                        {!commonState.collapsed ? <span style={{lineHeight: 1}}>Administrator</span> : null}
                    </p>
                </div>
            </Spin>
            <div className={styles.MenuItemDiv} style={{width: commonState.collapsed ? 50 : 250}}>
                <Spin spinning={state.menusLoads} indicator={<LoadingOutlined />}>
                    <div style={{width: '100%', minHeight: 100}}>
                        {!state.menusLoads ? <Menu
                            mode="inline"
                            inlineCollapsed={commonState.collapsed}
                            defaultOpenKeys={[menusActive.defaultOpenSelectKey]}
                            selectedKeys={[menusActive.defaultSelectedKeys]}
                            style={{ width: '100%' }}
                            items={state.menus}
                            onClick={menusClick}
                        /> : null}
            ...
            <div className={styles.MenuFoldOutIconDiv}>
                <MenuFoldOutlined className={styles.menuIconCircle} onClick={()=>{
                    set({...commonState, collapsed:!commonState.collapsed})
                }} />
            </div>
        </div>
    </div>
